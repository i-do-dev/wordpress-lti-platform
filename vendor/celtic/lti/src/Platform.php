...
class Platform
{
    ...
    protected function onAuthenticate()
    {
        ...
        if (isset($_SESSION['ceLTIc_lti_initiated_login'])) {
            ...
            $parameters = Util::getRequestParameters();
            if (
                $parameters['login_hint'] !== $login['login_hint'] ||
    ...
    private function handleAuthenticationRequest()
    {
        ...
        if ($this->ok) { 
            $url = Tool::$defaultTool->messageUrl;
            if (Activity::$toolUrl) {
                $url = Activity::$toolUrl;
            }
            $this->messageParameters = $this->addSignature($url, $this->messageParameters, 'POST', null, $parameters['nonce']);
        }
